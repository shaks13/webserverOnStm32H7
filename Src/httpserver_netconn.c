  ...
  if (recv_err == ERR_OK)
  {
    if (netconn_err(conn) == ERR_OK) 
    {
      ...
      if ((buflen >=5) && (strncmp(buf, "GET /", 5) == 0))
      {
        ...
        else if((strncmp(buf, "GET /api/status", 14) == 0))
		{
          printf("[get] api status");
		  netconn_write(conn, (const unsigned char*)(json), (size_t)12, NETCONN_NOCOPY);
		}
        else if((strncmp(buf, "GET /color.html?c=1", 19) == 0))
		{
		  printf("[get] button");
		  char *json="{button:\"ok\"}";
		  netconn_write(conn, (const unsigned char*)(json), (size_t)13, NETCONN_NOCOPY);
		}
        else 
        {
          /* Load Error page */
